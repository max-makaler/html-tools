...
?>\n`;
    ...
    // 4.1 Удаляем саму ссылку Privacy Policy (контент внутри <li> удалится, <li> останется)
    res = res.replace(/<a[^>]+page=politics[^>]*>[\s\S]*?<\/a>/gi, '');

    // 4.2 Заменяем ссылку User Agreement на PHP код
    res = res.replace(/<a[^>]+page=agreement[^>]*>[\s\S]*?<\/a>/gi, "<?php $cms->copyright(' '); ?>");
